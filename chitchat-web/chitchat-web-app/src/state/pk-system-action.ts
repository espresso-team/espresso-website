...
export const pkSystemAction = {
  ...
  randomPickImageId:
    () =>
    ({ getState, setState }: pkSystemApi) => {
      const newImageId = randomIntBetweenZeroAndXButNotY(
        getState().modelArrays.length,
        getState().curImageId,
      );
  ...
  isModelSelected:
    ...
    ({ getState }: pkSystemApi) => {
      if (getState().curModelName === 'AI角色') return false;
      return true;
    },
  cleanMessageList:
    () =>
  ...
  updateMessageList:
    (message: IMessage) =>
    ({ getState, setState }: pkSystemApi) => {
      const newMessageList: IMessage[] = JSON.parse(
        JSON.stringify(getState().messageList),
      );
          ...
          chatHistory.map((chat) => {
            ...
            const messageItem = {
              ...
              sender: {
                ...
                profile: {
                  username: isUser
                    ? getState().user.profile.username
                    : modelName,
                  avatar: isUser
                    ? getState().user.profile.gender === GenderType.FEMALE
                      ? 'https://chichat-images-1317940514.cos.ap-nanjing.myqcloud.com/static/WechatIMG4576.jpg'
                      : 'https://chichat-images-1317940514.cos.ap-nanjing.myqcloud.com/static/WechatIMG4577.jpg'
                    : modelSrc,
                },
              },
            } as IMessage;
